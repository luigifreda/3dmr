...
bool multiagent::isInCollision(const Eigen::Vector4d& start, const Eigen::Vector4d& end,
                               ...
                               const std::vector<std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d> >*>& agent_paths)
{
  for (typename std::vector<std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d> >*>::const_iterator it = agent_paths.begin();
...
std::vector<Eigen::Vector4d, Eigen::aligned_allocator<Eigen::Vector4d> > multiagent::circumnavigate(
    ...
    const std::vector<std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d> >*>& agent_paths)
{
  std::vector < Eigen::Vector4d, Eigen::aligned_allocator<Eigen::Vector4d> > ret;
  ROS_ERROR_THROTTLE(
      1,
