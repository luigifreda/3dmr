...
int main( int argc, char** argv )
{
  ...
  float control_frequency = getParam<float>(n, "control_rate", 40);
  float publish_rate = getParam<float>(n, "publish_rate", 10);

  ros::Timer cycle_timer = n.createTimer(ros::Duration(1./control_frequency), CB_mainCycle);
  ros::Timer publish_timer = n.createTimer(ros::Duration(1./publish_rate), CB_publishCycle);
...
//  obst_vector.push_back( boost::make_shared<PointObstacle>(-1.5,-0.5) );
  ...
  */
  ...
  config.robot_model = TebLocalPlannerROS::getRobotFootprintFromParamServer(n, config); //  new version 
#else   
...
void CB_mainCycle(const ros::TimerEvent& e)
{
  ...
  planner->plan(PoseSE2(-4,0,0), PoseSE2(4,0,0), &start_vel, free_goal_vel); // hardcoded start and goal for testing purposes
#else 
