...
const std::string PatrollingMarkerController::kDefaultMakerName = "patrolling_marker"; 
const std::string PatrollingMarkerController::kDefaultMakerDescription = "Patrolling"; 

const float PatrollingMarkerController::kInitialVerticalOffset = 1; 
const float PatrollingMarkerController::kTextMessageHeight = 0.14; 
const float PatrollingMarkerController::kTextMessageVerticalOffset = 0.5; 

const float PatrollingMarkerController::kPoseUpdatePeriod = 5; 

PatrollingMarkerController::PatrollingMarkerController(const tf::Vector3& p0, 
                                   const ros::NodeHandle& nh, 
                                   const std::string& ugv_name,                                                         
                                   const std::string& robot_frame_id, 
                                   const std::string& world_frame_id,                                   
                                   const std::string& patrol_pause_topic_name, 
    ...
    :nh_(nh)
{
    ...
    ROS_INFO_STREAM("********************************************************");
    ROS_INFO_STREAM("patrol_pause_topic_name: " << patrol_pause_topic_name );
    ROS_INFO_STREAM("********************************************************");
    patrolling_pause_pub_ = nh_.advertise<std_msgs::Bool>(patrol_pause_topic_name, 1);
    ...
    menu_handler_.insert("Pause Patrolling", boost::bind(&PatrollingMarkerController::processFeedback, this, _1));
    menu_handler_.insert("Restart Patrolling", boost::bind(&PatrollingMarkerController::processFeedback, this, _1));
    ...
    pose_timer_ =  nh_.createTimer(ros::Duration(kPoseUpdatePeriod), &PatrollingMarkerController::updateRobotPosition, this); // the timer will automatically fire at startup   
}

PatrollingMarkerController::~PatrollingMarkerController()
{
...
void PatrollingMarkerController::reset()
{
    boost::recursive_mutex::scoped_lock locker(marker_server_mtx); 
...
Marker PatrollingMarkerController::makeBox(InteractiveMarker& msg, const Color& color)
{
    Marker marker;
...
InteractiveMarkerControl& PatrollingMarkerController::makeBoxControl(InteractiveMarker& msg)
{
...
void PatrollingMarkerController::makeViewFacingMarker()
{
...
//    int_marker_.pose.position.z = 0;
    ...
    int_marker_.description = PatrollingMarkerController::kDefaultMakerDescription + " " + ugv_name_;
    ...
    marker_server_->setCallback(int_marker_.name, boost::bind(&PatrollingMarkerController::processFeedback, this, _1));
    menu_handler_.apply(*marker_server_, int_marker_.name);
    marker_server_->applyChanges();
    }
}

void PatrollingMarkerController::processFeedback(const visualization_msgs::InteractiveMarkerFeedbackConstPtr& feedback)
{
    std::ostringstream s;
    ...
    case visualization_msgs::InteractiveMarkerFeedback::MENU_SELECT:
        ...
        if (feedback->menu_entry_id == 1) // Pause Patrolling
        {
            ROS_INFO_STREAM("Pause Patrolling *******************************");
            std_msgs::Bool msg;
            msg.data = true; 
            patrolling_pause_pub_.publish(msg);
        }
        if (feedback->menu_entry_id == 2) // Restart Patrolling
        {
            ROS_INFO_STREAM("Restart Patrolling *****************************");
            std_msgs::Bool msg;
            msg.data = false; 
            patrolling_pause_pub_.publish(msg);
        }        
...
void PatrollingMarkerController::setMarkerPosition(const geometry_msgs::Pose &pose)
{
    boost::recursive_mutex::scoped_lock locker(marker_server_mtx); 
...
void PatrollingMarkerController::setMarkerColor(const Color& color, const std::string& text)
{
...
void PatrollingMarkerController::updateRobotPosition(const ros::TimerEvent& timer_msg)
{
