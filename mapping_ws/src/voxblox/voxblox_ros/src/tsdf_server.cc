...
TsdfServer::TsdfServer(const ros::NodeHandle& nh,
    ...
    : nh_(nh),
      ...
      publish_mesh_pointcloud_(false),
      cache_mesh_(false),
      enable_icp_(false),
  ...
  // Luigi: new point cloud 
  mesh_pointcloud_pub_ = 
      nh_private_.advertise<pcl::PointCloud<pcl::PointXYZRGBNormal>>(
      "mesh_pointcloud", 1, true);
  nh_private_.param("publish_mesh_pointcloud", publish_mesh_pointcloud_, publish_mesh_pointcloud_);
...
bool TsdfServer::getNextPointcloudFromQueue(
  ...
  *pointcloud_msg = queue->front();
  if (transformer_.lookupTransform((*pointcloud_msg)->header.frame_id,
    ...
    ROS_WARN_STREAM("cannot get transform between " << (*pointcloud_msg)->header.frame_id << " and " << world_frame_ << " at " << (*pointcloud_msg)->header.stamp); 
...
void TsdfServer::publishMeshAsPointcloud() {
...
void TsdfServer::updateMesh() {
  ...
  if(publish_mesh_pointcloud_) {
    publishMeshAsPointcloud();
  }
