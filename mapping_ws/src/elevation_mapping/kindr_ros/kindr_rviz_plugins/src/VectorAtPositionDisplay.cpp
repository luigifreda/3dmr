...
VectorAtPositionDisplay::VectorAtPositionDisplay()
    : lengthScale_(1.0f), widthScale_(1.0f), showText_(true), color_(Ogre::ColourValue::Black), alpha_(1.0) {
  connect(this, SIGNAL(updateVectorAtPositionSignal()), this, SLOT(updateVectorAtPosition()));

  length_scale_property_ = new rviz::FloatProperty("Length scale", 1.0, "Scale of the length of the vector.", this, SLOT(updateScale()));

  width_scale_property_ = new rviz::FloatProperty("Width scale", 1.0, "Scale of the width of the vector.", this, SLOT(updateScale()));
  width_scale_property_->setMin(0);

  show_text_property_ = new rviz::BoolProperty("Show text", true, "Enable or disable text rendering.", this, SLOT(updateShowText()));

  color_property_ = new rviz::ColorProperty("Color", QColor(0, 0, 0), "Color to draw the vector (if not defined by vector type).", this,
                                            SLOT(updateColorAndAlpha()));

  alpha_property_ =
      new rviz::FloatProperty("Alpha", 1.0, "0 is fully transparent, 1.0 is fully opaque.", this, SLOT(updateColorAndAlpha()));

  history_length_property_ =
      new rviz::IntProperty("History Length", 1, "Number of prior measurements to display.", this, SLOT(updateHistoryLength()));
...
void VectorAtPositionDisplay::updateVectorAtPosition() {
  ...
  if (current_vector_at_position_->position_frame_id.empty() ||
      current_vector_at_position_->position_frame_id == current_vector_at_position_->header.frame_id) {
    // Arrow and position frame coincide.

    // Get arrow position and orientation
    if (!context_->getFrameManager()->getTransform(current_vector_at_position_->header.frame_id, current_vector_at_position_->header.stamp,
                                                   positionFixedToPositionFrameInFixedFrame, orientationArrowFrameToFixedFrame)) {
      ROS_ERROR("Error transforming from frame '%s' to frame '%s'", current_vector_at_position_->position_frame_id.c_str(),
    ...
    if (!context_->getFrameManager()->getTransform(current_vector_at_position_->position_frame_id,
                                                   current_vector_at_position_->header.stamp, positionFixedToPositionFrameInFixedFrame,
    ...
    if (!context_->getFrameManager()->getTransform(current_vector_at_position_->header.frame_id, current_vector_at_position_->header.stamp,
                                                   dummyPosition, orientationArrowFrameToFixedFrame)) {
      ROS_ERROR("Error transforming from frame '%s' to frame '%s'", current_vector_at_position_->header.frame_id.c_str(),
                qPrintable(fixed_frame_));
      return;
    }
  }
  ...
  positionFixedToArrowInFixedFrame =
      positionFixedToPositionFrameInFixedFrame + rotMat * Ogre::Vector3(current_vector_at_position_->position.x,
