...
MultiDOFJointTrajectoryPointVisual::MultiDOFJointTrajectoryPointVisual(
    Ogre::SceneManager* scene_manager, Ogre::SceneNode* parent_node, const trajectory_msgs::MultiDOFJointTrajectoryPoint& msg,
    bool show_transform_rotation, bool show_velocity_linear, bool show_velocity_angular, bool show_acceleration_linear,
    bool show_acceleration_angular, float size_transform_rotation, float diameter_arrows, float scale_velocity_linear,
    float scale_velocity_angular, float scale_acceleration_linear, float scale_acceleration_angular, float alpha_transform_rotatation,
    const Ogre::ColourValue& color_velocity_linear, const Ogre::ColourValue& color_velocity_angular,
    const Ogre::ColourValue& color_acceleration_linear, const Ogre::ColourValue& color_acceleration_angular,
  ...
  assert((msg.transforms.size() == captions.size()) &&
         "ERROR: MultiDOFJointTrajectoryPointVisual: captions and transforms vectors do not have the same length.");
  assert((msg.velocities.size() == captions.size() || msg.velocities.size() == 0) &&
         "ERROR: MultiDOFJointTrajectoryPointVisual: velocity vector has invalid length.");
  assert((msg.accelerations.size() == captions.size() || msg.accelerations.size() == 0) &&
         "ERROR: MultiDOFJointTrajectoryPointVisual: acceleration vector has invalid length.");
  ...
  for (unsigned int i = 0; i < msg.transforms.size(); i++) {
    ...
    transforms_position_.back()->setPosition(msg.transforms[i].translation.x, msg.transforms[i].translation.y,
                                             msg.transforms[i].translation.z);

    // transform rotation
    const Ogre::Quaternion orientation(msg.transforms[i].rotation.w, msg.transforms[i].rotation.x, msg.transforms[i].rotation.y,
                                       msg.transforms[i].rotation.z);
    transforms_rotation_.push_back(boost::shared_ptr<rviz::Axes>(new rviz::Axes(
        scene_manager_, transforms_position_.back(), size_transform_rotation, axis_radius_per_size_ * size_transform_rotation)));
    ...
    if (message_contains_velocities) {
      ...
      velocities_linear_.push_back(boost::shared_ptr<rviz::Arrow>(
          new rviz::Arrow(scene_manager, transforms_position_.back(), 0.8 * scale_velocity_linear_ * velocities_linear_absolute_.back(),
                          diameter_arrows_, 0.2 * scale_velocity_linear_ * velocities_linear_absolute_.back(), 2.0 * diameter_arrows_)));
      ...
      velocities_angular_.push_back(boost::shared_ptr<rviz::Arrow>(
          new rviz::Arrow(scene_manager, transforms_position_.back(), 0.8 * scale_velocity_angular_ * velocities_angular_absolute_.back(),
                          diameter_arrows_, 0.2 * scale_velocity_angular_ * velocities_angular_absolute_.back(), 2.0 * diameter_arrows_)));
    ...
    if (message_contains_accelerations) {
      ...
      accelerations_linear_.push_back(boost::shared_ptr<rviz::Arrow>(new rviz::Arrow(
          scene_manager, transforms_position_.back(), 0.8 * scale_acceleration_linear_ * accelerations_linear_absolute_.back(),
          diameter_arrows_, 0.2 * scale_acceleration_linear_ * accelerations_linear_absolute_.back(), 2.0 * diameter_arrows_)));
      ...
      accelerations_angular_.push_back(boost::shared_ptr<rviz::Arrow>(new rviz::Arrow(
          scene_manager, transforms_position_.back(), 0.8 * scale_acceleration_angular_ * accelerations_angular_absolute_.back(),
          diameter_arrows_, 0.2 * scale_acceleration_angular_ * accelerations_angular_absolute_.back(), 2.0 * diameter_arrows_)));
...
void MultiDOFJointTrajectoryPointVisual::setCaptions(const std::vector<std::string>& captions) {
  ...
  assert(captions_.size() == captions.size() &&
         "ERROR: MultiDOFJointTrajectoryPointVisual: old and new captions vectors do not have the same length.");
...
void MultiDOFJointTrajectoryPointVisual::updateScaleVelocityLinear() {
  for (unsigned int i = 0; i < velocities_linear_.size(); i++) {
    velocities_linear_[i]->set(0.8 * scale_velocity_linear_ * velocities_linear_absolute_[i], diameter_arrows_,
                               0.2 * scale_velocity_linear_ * velocities_linear_absolute_[i], 2.0 * diameter_arrows_);
  }
}

void MultiDOFJointTrajectoryPointVisual::updateScaleVelocityAngular() {
  for (unsigned int i = 0; i < velocities_angular_.size(); i++) {
    velocities_angular_[i]->set(0.8 * scale_velocity_angular_ * velocities_angular_absolute_[i], diameter_arrows_,
                                0.2 * scale_velocity_angular_ * velocities_angular_absolute_[i], 2.0 * diameter_arrows_);
  }
}

void MultiDOFJointTrajectoryPointVisual::updateScaleAccelerationLinear() {
  for (unsigned int i = 0; i < accelerations_linear_.size(); i++) {
    accelerations_linear_[i]->set(0.8 * scale_acceleration_linear_ * accelerations_linear_absolute_[i], diameter_arrows_,
                                  0.2 * scale_acceleration_linear_ * accelerations_linear_absolute_[i], 2.0 * diameter_arrows_);
  }
}
...
void MultiDOFJointTrajectoryPointVisual::updateScaleAccelerationAngular() {
  for (unsigned int i = 0; i < accelerations_angular_.size(); i++) {
    accelerations_angular_[i]->set(0.8 * scale_acceleration_angular_ * accelerations_angular_absolute_[i], diameter_arrows_,
                                   0.2 * scale_acceleration_angular_ * accelerations_angular_absolute_[i], 2.0 * diameter_arrows_);
  }
}
