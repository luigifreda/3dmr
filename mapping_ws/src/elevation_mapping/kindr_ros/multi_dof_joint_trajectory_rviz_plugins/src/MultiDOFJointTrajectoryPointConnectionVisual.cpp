...
MultiDOFJointTrajectoryPointConnectionVisual::MultiDOFJointTrajectoryPointConnectionVisual(
    Ogre::SceneManager* scene_manager, Ogre::SceneNode* parent_node, const trajectory_msgs::MultiDOFJointTrajectoryPoint& from,
    const trajectory_msgs::MultiDOFJointTrajectoryPoint& to, float show_connection, const Ogre::ColourValue& color)
    : scene_manager_(scene_manager), show_connection_(show_connection), color_(color) {
  // check vector lengths
  assert(from.transforms.size() == to.transforms.size() &&
         "ERROR: MultiDOFJointTrajectoryPointConnectionVisual: from and to vectors do not have the same length.");
